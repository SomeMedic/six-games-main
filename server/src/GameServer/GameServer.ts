...
import {AuthPayload, ClientToServerEvents, GameRules, ServerToClientEvents, User} from "./DataModel";
import {AuthError, RoomNotFoundError} from "./Errors";
import {ServerRoom} from "./ServerRoom";
import {ChessServerRoom} from "../Games/ChessServerRoom";
import {catchErrors} from "./SocketErrorHandler";
...
export class GameServer {
    ...
    public createRoom = (host: User, gameRules: GameRules, id?: string): ServerRoom => {
        ...
        if (gameRules.type === "chess") {
            room = new ChessServerRoom(host, gameRules, id);
        } else {
            room = new ServerRoom(host, gameRules, id);
        }
